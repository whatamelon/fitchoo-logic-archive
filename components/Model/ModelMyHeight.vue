...
created() {
 ...
 methods: {
    async handleScroll() {
      ...
      if (
        this.$route.name=="model" &&
        this.$store.getters.MODELS.length !=0 &&
        window.innerHeight + window.scrollY >= document.body.offsetHeight-100 &&
        !this.isSpinning &&
        this.moreModel
       ) {
         this.isSpinning = true;
         await this.getMoreModels();
      console.log("내키 헨들스크롤움직임");
       }
    },
    ...
    getMoreModels() {
      return new Promise(async (resolve, reject) => {
        try {
            ...
            const noMoreModel = response.result.modelList;
            if (noMoreModel.length == 0) {
